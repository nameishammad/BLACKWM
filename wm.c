...
typedef struct {
  ...
  int workspace_win_fullscreen[MAX_WORKSPACES][MAX_WINDOWS];
  int workspace_win_fullscreen_prev_maximized[MAX_WORKSPACES][MAX_WINDOWS];
  int16_t workspace_win_prefullscreen_x[MAX_WORKSPACES][MAX_WINDOWS];
  int16_t workspace_win_prefullscreen_y[MAX_WORKSPACES][MAX_WINDOWS];
  uint16_t workspace_win_prefullscreen_w[MAX_WORKSPACES][MAX_WINDOWS];
  uint16_t workspace_win_prefullscreen_h[MAX_WORKSPACES][MAX_WINDOWS];
...
void get_true_fullscreen_target(int16_t *target_x, int16_t *target_y,
                                uint16_t *target_w, uint16_t *target_h);
static int window_is_fullscreen(xcb_window_t win);
static void raise_fullscreen_window_for_workspace(int ws);
static void configure_fullscreen_window(int ws, int idx);
...
xcb_atom_t A_NET_WM_STATE_FULLSCREEN;
...
void init_ewmh_atoms() {
  ...
  A_NET_WM_STATE = intern_atom("_NET_WM_STATE");
  A_NET_WM_STATE_FULLSCREEN = intern_atom("_NET_WM_STATE_FULLSCREEN");
  ...
  xcb_atom_t supported[] = {A_NET_CLIENT_LIST,        A_NET_WM_DESKTOP,
                            ...
                            A_NET_ACTIVE_WINDOW,      A_NET_WM_STATE,
...
void init_workspaces() {
  for (int i = 0; i < MAX_WORKSPACES; i++) {
    ...
    for (int j = 0; j < MAX_WINDOWS; j++) {
      ...
      wm.workspace_win_fullscreen[i][j] = 0;
      wm.workspace_win_fullscreen_prev_maximized[i][j] = 0;
      wm.workspace_win_prefullscreen_x[i][j] = 0;
      wm.workspace_win_prefullscreen_y[i][j] = 0;
      wm.workspace_win_prefullscreen_w[i][j] = 0;
      wm.workspace_win_prefullscreen_h[i][j] = 0;
...
void raise_docks() {
  ...
  /* Fullscreen is deliberately above panels/docks. */
  raise_fullscreen_window_for_workspace(wm.current_workspace);
}
...
void get_fullscreen_target(int16_t *target_x, int16_t *target_y,
                           uint16_t *target_w, uint16_t *target_h) {
...
/* True X11 fullscreen: cover the entire monitor, including docks/panels.
 * get_fullscreen_target() above is intentionally kept for normal
 * maximize/reel geometry because it respects the usable area and gaps. */
void get_true_fullscreen_target(int16_t *target_x, int16_t *target_y,
                                uint16_t *target_w, uint16_t *target_h) {
  *target_x = 0;
  *target_y = 0;
  *target_w = (wm.screen_width > 0) ? (uint16_t)wm.screen_width : 1;
  *target_h = (wm.screen_height > 0) ? (uint16_t)wm.screen_height : 1;
}

static int fullscreen_index(int ws, xcb_window_t win) {
  if (ws < 0 || ws >= MAX_WORKSPACES || win == XCB_WINDOW_NONE)
    return -1;
  int idx = find_window_idx(ws, win);
  if (idx >= 0 && wm.workspace_win_fullscreen[ws][idx])
    return idx;
  return -1;
}
...
static int window_is_fullscreen(xcb_window_t win) {
  ...
  for (int ws = 0; ws < MAX_WORKSPACES; ws++) {
    if (fullscreen_index(ws, win) >= 0)
      return 1;
  }
  return 0;
}

static int window_has_fullscreen_state(xcb_window_t win) {
  if (win == XCB_WINDOW_NONE || A_NET_WM_STATE == XCB_ATOM_NONE ||
      A_NET_WM_STATE_FULLSCREEN == XCB_ATOM_NONE)
    return 0;

  xcb_get_property_cookie_t c =
      xcb_get_property(wm.conn, 0, win, A_NET_WM_STATE, XCB_ATOM_ATOM, 0, 64);
...
static void set_fullscreen_state_property(xcb_window_t win, int enabled) {
  if (win == XCB_WINDOW_NONE || A_NET_WM_STATE == XCB_ATOM_NONE ||
      A_NET_WM_STATE_FULLSCREEN == XCB_ATOM_NONE)
    return;
  ...
  if (enabled) {
    ...
    if (r && r->type == XCB_ATOM_ATOM && r->format == 32) {
      ...
      if (count > 64)
        count = 64;
      xcb_atom_t *old = (xcb_atom_t *)xcb_get_property_value(r);
      for (int i = 0; i < count; i++) {
        atoms[n++] = old[i];
        if (old[i] == A_NET_WM_STATE_FULLSCREEN)
          already = 1;
      }
    }
    if (r)
    ...
    if (!already && n < 64)
      atoms[n++] = A_NET_WM_STATE_FULLSCREEN;

    xcb_change_property(wm.conn, XCB_PROP_MODE_REPLACE, win, A_NET_WM_STATE,
                        XCB_ATOM_ATOM, 32, n, atoms);
  } else {
    xcb_get_property_cookie_t c =
        xcb_get_property(wm.conn, 0, win, A_NET_WM_STATE, XCB_ATOM_ATOM, 0, 64);
    xcb_get_property_reply_t *r = xcb_get_property_reply(wm.conn, c, NULL);
    ...
    if (r && r->type == XCB_ATOM_ATOM && r->format == 32) {
      ...
      xcb_atom_t *old = (xcb_atom_t *)xcb_get_property_value(r);
      for (int i = 0; i < count; i++) {
        if (old[i] != A_NET_WM_STATE_FULLSCREEN && n < 64)
          atoms[n++] = old[i];
      }
      xcb_change_property(wm.conn, XCB_PROP_MODE_REPLACE, win, A_NET_WM_STATE,
                          XCB_ATOM_ATOM, 32, n, atoms);
    } else {
      xcb_delete_property(wm.conn, win, A_NET_WM_STATE);
    }
    if (r)
      free(r);
  }
...
static void raise_fullscreen_window_for_workspace(int ws) {
  ...
  for (int i = 0; i < wm.workspace_counts[ws]; i++) {
    if (!wm.workspace_win_fullscreen[ws][i])
      continue;
...
void tile_layout(int ws, TileNode *node, int16_t x, int16_t y, uint16_t w,
  ...
  if (node->is_leaf) {
    ...
    if (idx < 0)
      return;
    if (wm.workspace_win_fullscreen[ws][idx])
      return;
...
void apply_tiling_geometry(int ws) {
  ...
  for (int i = 0; i < count; i++) {
    ...
    if (wm.workspace_win_fullscreen[ws][i]) {
      int16_t fx, fy;
      uint16_t fw, fh;
      get_true_fullscreen_target(&fx, &fy, &fw, &fh);
      uint32_t fvalues[] = {(uint32_t)(int32_t)fx, (uint32_t)(int32_t)fy, fw,
                            fh};
...
void compute_reel_layout(int ws) {
  ...
  get_fullscreen_target(&target_x, &target_y, &target_w, &target_h);
...
void apply_reel_geometry_parked(int ws) {
  ...
  for (int i = 0; i < count; i++) {
    ...
    if (wm.workspace_win_fullscreen[ws][i]) {
      int32_t park_x = wm.screen_width * (ws + 2);
      uint32_t fmask = XCB_CONFIG_WINDOW_X | XCB_CONFIG_WINDOW_Y |
                       XCB_CONFIG_WINDOW_WIDTH | XCB_CONFIG_WINDOW_HEIGHT;
      uint32_t fvalues[] = {(uint32_t)(int32_t)park_x, 0, (uint32_t)w,
                            (uint32_t)h};
      xcb_configure_window(wm.conn, win, fmask, fvalues);
      continue;
    }
...
void swap_reel_windows(int ws, int idx_a, int idx_b) {
  ...
  int tmp_fs = wm.workspace_win_fullscreen[ws][idx_a];
  wm.workspace_win_fullscreen[ws][idx_a] =
      wm.workspace_win_fullscreen[ws][idx_b];
  wm.workspace_win_fullscreen[ws][idx_b] = tmp_fs;
  int tmp_fspm = wm.workspace_win_fullscreen_prev_maximized[ws][idx_a];
  wm.workspace_win_fullscreen_prev_maximized[ws][idx_a] =
      wm.workspace_win_fullscreen_prev_maximized[ws][idx_b];
  wm.workspace_win_fullscreen_prev_maximized[ws][idx_b] = tmp_fspm;
...
static void configure_fullscreen_window(int ws, int idx) {
  if (ws < 0 || ws >= MAX_WORKSPACES || idx < 0 ||
      idx >= wm.workspace_counts[ws])
    return;
  ...
  uint16_t fw, fh;
  get_true_fullscreen_target(&fx, &fy, &fw, &fh);
  uint32_t values[] = {(uint32_t)(int32_t)fx, (uint32_t)(int32_t)fy, fw, fh};
  xcb_configure_window(wm.conn, win,
                       XCB_CONFIG_WINDOW_X | XCB_CONFIG_WINDOW_Y |
                           XCB_CONFIG_WINDOW_WIDTH | XCB_CONFIG_WINDOW_HEIGHT,
                       values);
...
static void enter_fullscreen_window(int ws, int idx, int notify_user) {
  if (ws < 0 || ws >= MAX_WORKSPACES || idx < 0 ||
      idx >= wm.workspace_counts[ws])
    return;

  xcb_window_t win = wm.workspace_windows[ws][idx];
  if (win == XCB_WINDOW_NONE || wm.workspace_win_fullscreen[ws][idx])
    return;
  ...
  if (geo) {
    wm.workspace_win_prefullscreen_x[ws][idx] = geo->x;
    wm.workspace_win_prefullscreen_y[ws][idx] = geo->y;
    wm.workspace_win_prefullscreen_w[ws][idx] = geo->width;
    wm.workspace_win_prefullscreen_h[ws][idx] = geo->height;
    free(geo);
  } else {
    wm.workspace_win_prefullscreen_x[ws][idx] =
        (int16_t)wm.workspace_win_x[ws][idx];
    wm.workspace_win_prefullscreen_y[ws][idx] =
        (int16_t)wm.workspace_win_y[ws][idx];
    wm.workspace_win_prefullscreen_w[ws][idx] =
        (uint16_t)((wm.workspace_win_w[ws][idx] > 0)
                       ? wm.workspace_win_w[ws][idx]
                       : 1);
    wm.workspace_win_prefullscreen_h[ws][idx] =
        (uint16_t)((wm.workspace_win_h[ws][idx] > 0)
                       ? wm.workspace_win_h[ws][idx]
                       : 1);
  }

  wm.workspace_win_fullscreen_prev_maximized[ws][idx] =
      wm.workspace_win_maximized[ws][idx];
  wm.workspace_win_fullscreen[ws][idx] = 1;

  set_fullscreen_state_property(win, 1);
  configure_fullscreen_window(ws, idx);
  xcb_flush(wm.conn);

  if (notify_user)
    send_notify("Fullscreen ON");
}

static void exit_fullscreen_window(int ws, int idx, int notify_user) {
  if (ws < 0 || ws >= MAX_WORKSPACES || idx < 0 ||
      idx >= wm.workspace_counts[ws])
    return;
  ...
  wm.workspace_win_fullscreen[ws][idx] = 0;
  set_fullscreen_state_property(win, 0);
  ...
  if (wm.workspace_tiling[ws]) {
    compute_tiling_layout(ws);
    apply_tiling_geometry(ws);
  } else if (wm.workspace_floating[ws]) {
    wm.workspace_win_x[ws][idx] = wm.workspace_win_prefullscreen_x[ws][idx];
    wm.workspace_win_y[ws][idx] = wm.workspace_win_prefullscreen_y[ws][idx];
    wm.workspace_win_w[ws][idx] = wm.workspace_win_prefullscreen_w[ws][idx];
    wm.workspace_win_h[ws][idx] = wm.workspace_win_prefullscreen_h[ws][idx];
    compute_floating_layout(ws);
    apply_reel_geometry(ws);
  } else {
    compute_reel_layout(ws);
  ...
  if (notify_user)
    send_notify("Fullscreen OFF");
}

static void toggle_fullscreen_window(void) {
  ...
  if (wm.workspace_win_fullscreen[ws][idx])
    exit_fullscreen_window(ws, idx, 1);
  else
    enter_fullscreen_window(ws, idx, 1);

  focus_window(wm.workspace_windows[ws][idx]);
  raise_fullscreen_window_for_workspace(ws);
  xcb_flush(wm.conn);
}

static void sync_fullscreen_from_property(xcb_window_t win) {
  ...
  int requested = window_has_fullscreen_state(win);
  if (requested && !wm.workspace_win_fullscreen[ws][idx]) {
    if (ws != wm.current_workspace)
      switch_workspace(ws);
    enter_fullscreen_window(ws, idx, 0);
    focus_window(win);
    raise_fullscreen_window_for_workspace(ws);
  } else if (!requested && wm.workspace_win_fullscreen[ws][idx]) {
    exit_fullscreen_window(ws, idx, 0);
  }
}
...
void cycle_focus(int ws, int direction) {
  if (window_is_fullscreen(wm.focused))
    return;
...
void add_window_to_workspace_ext(xcb_window_t win, int ws, int force_float) {
  ...
  if (wm.workspace_counts[ws] < MAX_WINDOWS) {
    ...
    wm.workspace_win_maximized[ws][idx] = 0;
    wm.workspace_win_fullscreen[ws][idx] = 0;
    wm.workspace_win_fullscreen_prev_maximized[ws][idx] = 0;
    wm.workspace_win_prefullscreen_x[ws][idx] = 0;
    wm.workspace_win_prefullscreen_y[ws][idx] = 0;
    wm.workspace_win_prefullscreen_w[ws][idx] = 0;
    wm.workspace_win_prefullscreen_h[ws][idx] = 0;
    ...
    for (int j = 0; j < wm.workspace_counts[i]; j++) {
      if (wm.workspace_windows[i][j] == win) {
        ...
        for (int k = j; k < wm.workspace_counts[i] - 1; k++) {
          ...
          wm.workspace_win_fullscreen[i][k] =
              wm.workspace_win_fullscreen[i][k + 1];
          wm.workspace_win_fullscreen_prev_maximized[i][k] =
              wm.workspace_win_fullscreen_prev_maximized[i][k + 1];
          wm.workspace_win_prefullscreen_x[i][k] =
              wm.workspace_win_prefullscreen_x[i][k + 1];
          wm.workspace_win_prefullscreen_y[i][k] =
              wm.workspace_win_prefullscreen_y[i][k + 1];
          wm.workspace_win_prefullscreen_w[i][k] =
              wm.workspace_win_prefullscreen_w[i][k + 1];
          wm.workspace_win_prefullscreen_h[i][k] =
              wm.workspace_win_prefullscreen_h[i][k + 1];
        ...
        wm.workspace_win_fullscreen[i][wm.workspace_counts[i]] = 0;
        wm.workspace_win_fullscreen_prev_maximized[i][wm.workspace_counts[i]] =
            0;
...
void switch_workspace(int workspace) {
  ...
  for (int i = 0; i < from_count; i++) {
    if (!wm.workspace_tiling[from_ws] && !wm.workspace_floating[from_ws]) {
      ...
      if (wm.workspace_win_fullscreen[from_ws][i]) {
        from_x[i] = wm.workspace_win_prefullscreen_x[from_ws][i];
        from_y[i] = wm.workspace_win_prefullscreen_y[from_ws][i];
        from_w[i] = (int16_t)wm.workspace_win_prefullscreen_w[from_ws][i];
        continue;
      }
      xcb_get_geometry_cookie_t gc = xcb_get_geometry(wm.conn, win);
      xcb_get_geometry_reply_t *geo = xcb_get_geometry_reply(wm.conn, gc, NULL);
  ...
  for (int i = 0; i < to_count; i++) {
    ...
    if (wm.workspace_win_fullscreen[to_ws][i]) {
      int16_t fx, fy;
      uint16_t fw, fh;
      get_true_fullscreen_target(&fx, &fy, &fw, &fh);
      to_target_x[i] = fx;
  ...
  for (int i = 0; i < from_count; i++) {
    ...
    if (wm.workspace_win_fullscreen[from_ws][i]) {
      uint32_t fmask = XCB_CONFIG_WINDOW_X | XCB_CONFIG_WINDOW_Y;
      int32_t fvalues[] = {wm.screen_width * (from_ws + 2), 0};
      xcb_configure_window(wm.conn, win, fmask, (uint32_t *)fvalues);
      continue;
    }
...
void focus_window(xcb_window_t win) {
  ...
  if (wm.focused != XCB_WINDOW_NONE && wm.focused != win &&
      window_is_fullscreen(wm.focused) && !is_dialog_window(win)) {
    /* Do not move keyboard focus behind an active fullscreen client. */
    return;
  }
  ...
  xcb_configure_window(
      ...
      (uint32_t[]){window_is_fullscreen(win) ? 0 : BORDER_WIDTH});
  xcb_flush(wm.conn);
...
void handle_map_request(xcb_map_request_event_t *ev) {
  ...
  if (win_is_dialog) {
    ...
    if (prev_focus != XCB_WINDOW_NONE) {
      ...
      if (ngeo && ogeo) {
        int16_t nx = ngeo->x, ny = ngeo->y;
        uint16_t nw = ngeo->width, nh = ngeo->height;
        int16_t ox = ogeo->x, oy = ogeo->y;
        uint16_t ow = ogeo->width, oh = ogeo->height;

        int dx = abs((int)nx - (int)ox);
        int dy = abs((int)ny - (int)oy);
        int significant_overlap =
            (dx < (int)(ow / 4)) && (dy < (int)(oh / 4)) &&
            nw >= (uint16_t)(ow * 0.7) && nh >= (uint16_t)(oh * 0.7);

        if (significant_overlap) {
          int dlg_count_for_ws = 0;
          for (int i = 0; i < wm.dialog_count; i++) {
            if (wm.dialog_owner_ws[i] == wm.current_workspace)
              dlg_count_for_ws++;
          }
          int cascade_slot = (dlg_count_for_ws + 1) % 8;
          int16_t cascade_dx = (int16_t)(cascade_slot * 36);
          int16_t cascade_dy = (int16_t)(cascade_slot * 28);

          uint32_t pmask = XCB_CONFIG_WINDOW_X | XCB_CONFIG_WINDOW_Y;
          int32_t pvalues[] = {nx + cascade_dx, ny + cascade_dy};
  ...
  if (window_has_fullscreen_state(win)) {
    enter_fullscreen_window(ws, idx, 0);
  }

  compute_reel_layout(ws);

  if (wm.workspace_win_fullscreen[ws][idx] && ws == wm.current_workspace) {
    configure_fullscreen_window(ws, idx);
    focus_window(win);
    if (wm.in_overview)
      render_overview();
  ...
  if (ws != wm.current_workspace) {
    ...
    if (wm.workspace_floating[ws] || force_float) {
      ...
      int16_t fx, fy;
      get_fullscreen_target(&fx, &fy, &target_w, &target_h);
    }
...
void move_focused_window_to_workspace(int target_ws) {
  ...
  int saved_fullscreen = wm.workspace_win_fullscreen[from_ws][from_idx];
  int saved_fs_prev_max =
      wm.workspace_win_fullscreen_prev_maximized[from_ws][from_idx];
  int16_t saved_pre_fs_x = wm.workspace_win_prefullscreen_x[from_ws][from_idx];
  int16_t saved_pre_fs_y = wm.workspace_win_prefullscreen_y[from_ws][from_idx];
  uint16_t saved_pre_fs_w = wm.workspace_win_prefullscreen_w[from_ws][from_idx];
  uint16_t saved_pre_fs_h = wm.workspace_win_prefullscreen_h[from_ws][from_idx];
  ...
  if (wm.workspace_counts[target_ws] < MAX_WINDOWS) {
    ...
    wm.workspace_win_fullscreen[target_ws][new_idx] = saved_fullscreen;
    wm.workspace_win_fullscreen_prev_maximized[target_ws][new_idx] =
        saved_fs_prev_max;
    wm.workspace_win_prefullscreen_x[target_ws][new_idx] = saved_pre_fs_x;
    wm.workspace_win_prefullscreen_y[target_ws][new_idx] = saved_pre_fs_y;
    wm.workspace_win_prefullscreen_w[target_ws][new_idx] = saved_pre_fs_w;
    wm.workspace_win_prefullscreen_h[target_ws][new_idx] = saved_pre_fs_h;
...
void handle_client_message(xcb_client_message_event_t *ev) {
  if (ev->type == A_NET_WM_STATE && ev->format == 32 &&
      (ev->data.data32[1] == A_NET_WM_STATE_FULLSCREEN ||
       ev->data.data32[2] == A_NET_WM_STATE_FULLSCREEN)) {
    int ws = -1;
    int idx = -1;
    for (int i = 0; i < MAX_WORKSPACES && ws < 0; i++) {
      for (int j = 0; j < wm.workspace_counts[i]; j++) {
        if (wm.workspace_windows[i][j] == ev->window) {
          ws = i;
          idx = j;
          break;
        }
    ...
    if (ws >= 0 && idx >= 0) {
      uint32_t action = ev->data.data32[0]; /* 0 remove, 1 add, 2 toggle */
      if (ws != wm.current_workspace)
        switch_workspace(ws);
...
void handle_key_press(xcb_key_press_event_t *ev) {
  ...
  /* Mod+F: real X11 fullscreen.  Shift+Mod+F remains the floating-mode
   * toggle, so the existing mode shortcut is preserved. */
...
void handle_button_press(xcb_button_press_event_t *ev) {
  if (!wm.in_overview && (ev->state & MOVE_RESIZE_MOD) &&
    ...
    if (window_is_fullscreen(ev->child)) {
      focus_window(ev->child);
      return;
    }
...
void handle_motion_notify(xcb_motion_notify_event_t *ev) {
  ...
  if (window_is_fullscreen(wm.drag_window))
    return;
        ...
        if (geo) {
          if (idx >= 0) {
            ...
            if (!wm.workspace_floating[ws]) {
              int16_t tx, ty;
              uint16_t tw, th;
              get_fullscreen_target(&tx, &ty, &tw, &th);
...
void setup_key_grabs() {
  ...
  if (f_keycode) {
    grab_key_mod4_variants(*f_keycode, XCB_MOD_MASK_4);
...
void handle_configure_request(xcb_configure_request_event_t *ev) {
  for (int ws = 0; ws < MAX_WORKSPACES; ws++) {
    ...
    if (wm.workspace_win_fullscreen[ws][idx]) {
      /* A fullscreen client may issue ConfigureRequests while entering or
       * changing video/UI state.  Never let those requests break fullscreen. */
      if (ev->value_mask & XCB_CONFIG_WINDOW_STACK_MODE) {
        xcb_configure_window(wm.conn, ev->window, XCB_CONFIG_WINDOW_STACK_MODE,
                             &ev->stack_mode);
      }
      configure_fullscreen_window(ws, idx);
      xcb_flush(wm.conn);
...
void handle_property_notify(xcb_property_notify_event_t *ev) {
  if (ev->atom == A_NET_WM_STATE) {
    sync_fullscreen_from_property(ev->window);
    return;
  }
