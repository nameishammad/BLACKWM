...
int is_utility_window(xcb_window_t win) {
  ...
  if (cr && cr->type == XCB_ATOM_STRING && cr->format == 8) {
    ...
    // "gtk-picker",
    //                                "spectacle"};
    const char *utility_names[] = {"picom", "plank", "xfce4-popup-whiskermenu",
                                   "spectacle"};
  ...
  for (int i = 0; i < count; i++) {
    ...
    if (is_new) {
      ...
      if (geo) {
        ...
        wm.workspace_win_x[ws][i] =
            center_x - (geo->width / 2);
        wm.workspace_win_y[ws][i] =
            center_y - (geo->height / 2);
        free(geo);
      } else {
...
void compute_reel_layout(int ws) {
  ...
  for (int i = 0; i < count; i++) {
    ...
    if (wm.workspace_win_force_float[ws][i]) {
      wm.workspace_win_x[ws][i] =
          slot_x + wm.workspace_win_slide_dx[ws][i];
      wm.workspace_win_y[ws][i] =
          slot_y + wm.workspace_win_slide_dy[ws][i];
    } else {
...
void apply_reel_geometry(int ws) {
  ...
  for (int i = 0; i < count; i++) {
    ...
    uint16_t h = (wm.workspace_win_h[ws][i] > 0)
                     ...
                     uint32_t values[] = {
                       (uint32_t)(int32_t)wm.workspace_win_x[ws][i],
                       (uint32_t)(int32_t)wm.workspace_win_y[ws][i],
                       w,
                       h
                     };
...
void maximize_focused_window() {
  ...
  uint32_t mask = XCB_CONFIG_WINDOW_X | XCB_CONFIG_WINDOW_Y |
                  XCB_CONFIG_WINDOW_WIDTH | XCB_CONFIG_WINDOW_HEIGHT;
  uint32_t values[] = {(uint32_t)(int32_t)target_x,
                       (uint32_t)(int32_t)target_y, target_w, target_h};
  xcb_configure_window(wm.conn, win, mask, values);
  xcb_flush(wm.conn);
...
void switch_workspace(int workspace) {
  ...
  for (int i = 0; i < from_count; i++) {
    if (!wm.workspace_tiling[from_ws] &&
        !wm.workspace_floating[from_ws]) {
  ...
  /*
   * Restore the destination from canonical state after the transition.
   * This also repairs any temporary X geometry used during parking.
   */
  if (wm.workspace_counts[to_ws] > 0 &&
      !wm.workspace_tiling[to_ws] &&
...

  {
    ...
    for (int i = 0; i < wm.workspace_counts[cws]; i++) {
      ...
      if (wm.workspace_win_force_float[cws][i] &&
        ...
        xcb_configure_window(wm.conn, ffwin,
                             XCB_CONFIG_WINDOW_STACK_MODE,
                             (uint32_t[]){XCB_STACK_MODE_ABOVE});
      }
    }
...
void handle_map_request(xcb_map_request_event_t *ev) {
  ...
  if (win_is_dialog) {
    ...
    // POPUP-OVERLAP FIX: kai baar Firefox jaisi apps naya link/window
    // (window.open / target=_blank) kholte waqt naye window ka
    // WM_TRANSIENT_FOR apne hi main window pe set kar deti hain, isi
    // wajah se is_dialog_window() ise "dialog" samajh leta hai. Aise
    // windows ka client apni marzi ki position use karta hai, jo aksar
    // "owner" (yahan Firefox) ke bilkul upar/same jagah hoti hai — is
    // se owner window poora dhak jaata hai aur "hide" jaisa lagta hai.
    // Fix: agar naye window ki geometry uske owner (prev_focus) ki
    // geometry se substantially overlap karti hai, to isay thoda
    // cascade-offset kar dete hain taake owner peeche se dikhta rahe.
    if (prev_focus != XCB_WINDOW_NONE) {
      xcb_get_geometry_cookie_t ngc = xcb_get_geometry(wm.conn, win);
      xcb_get_geometry_reply_t *ngeo = xcb_get_geometry_reply(wm.conn, ngc, NULL);
      xcb_get_geometry_cookie_t ogc = xcb_get_geometry(wm.conn, prev_focus);
      xcb_get_geometry_reply_t *ogeo = xcb_get_geometry_reply(wm.conn, ogc, NULL);
  ...
  if (geo && (wm.workspace_floating[ws] || force_float)) {
    ...
    // Always open new floating windows at the exact center.
    // Do not cascade subsequent windows to the right/bottom.
    wm.workspace_win_x[ws][idx] = center_x - (geo->width / 2);
    wm.workspace_win_y[ws][idx] = center_y - (geo->height / 2);
  }
  if (geo) {
  ...
  // *** FLOATING MODE FIX ENDS HERE ***

  compute_reel_layout(ws);

  if (ws != wm.current_workspace) {
    // Doosre workspace pe khula hai (e.g. GIMP/Inkscape -> tag 8) — abhi
    // active screen pe nahi dikhna chahiye, isliye off-screen park karo,
    // switch_workspace() ke parking convention ki tarah.
...
// CHANGE: Pehle yahan resolve_dialog_move_owner()/move_owned_dialogs()
// hote the jo kisi dialog/popup (polkit/auth-agent, file-chooser
// waghera) ka "asal requesting app" dhoondh kar use MOVE ke waqt uske
// owner ke saath group kar dete the. Ab dialogs aur unki owner app
// jaan-boojh kar ek-doosre se MUSTAQIL (independent) rakhi jati hain:
// koi bhi window (app ho ya dialog/popup) doosre workspace mein sirf
// TABHI move hoti hai jab use khud target kiya jaye - overview
// drag-drop mein dono alag-alag drop targets par ja sakte hain, aur
// Mod+Shift+1..4 bhi sirf currently-focused window ko move karta hai,
// uski dialogs ya uske owner ko nahi.
...
void move_focused_window_to_workspace(int target_ws) {
  ...
  // FIX: agar khud focused window ek dialog/popup hai (polkit password
  // prompt, copy-progress popup waghera), ye wm.workspace_windows[]
  // array mein KABHI track nahi hoti (wo apni alag wm.dialog_windows[]
  // list mein rehti hai) - is liye neeche wala from_idx lookup ise kabhi
  // nahi milta tha aur function chup-chaap return ho jata: Mod+Shift+N
  // dialog par focus hote hue kuch nahi karta tha.
  //
  // CHANGE: pehle yahan dialog ko "akele" move karne ke bajaye uske
  // OWNER app-window ko move kiya jata tha (taake dono saath jayein).
  // Ab jaan-boojh kar dialog ko hamesha KHUD, akele/mustaqil move
  // karte hain - uska owner app jahan hai wahin rehta hai. Isse
  // popup/polkit dialog aur uski parent app Mod+Shift+N se bhi
  // ALAG-ALAG target workspaces par move ho sakte hain.
  for (int i = 0; i < wm.dialog_count; i++) {
  ...
  // CHANGE: pehle yahan is app-window ki saari owned dialogs
  // (move_owned_dialogs) bhi zabardasti saath target_ws move ho jati
  // thin. Ab jaan-boojh kar AISA NAHI karte - popup/polkit dialog aur
  // uski "parent" app ab overview drag-drop se ya Mod+Shift+N se
  // MUSTAQIL (independent) taur par move hoti hain: app move karo to
  // dialog wahin purane workspace mein rehti hai (aur vice versa),
  // jab tak dialog ko khud alag se move na kiya jaye.
  remove_window_from_workspace(win);
...
void move_window_to_workspace(xcb_window_t win, int target_ws) {
  ...
  /*
   * While overview is open, do NOT apply normal reel geometry here.
   * Inactive workspaces are parked and overview uses their stored layout.
   * Re-applying reel geometry to both source and target workspaces after
   * every DnD can bring parked windows back into the real workspace
   * coordinate space and makes subsequent overview thumbnails disappear.
   * render_overview() below is enough to refresh the workspace boxes.
   */
...
void handle_key_press(xcb_key_press_event_t *ev) {
  ...
  if ((ev->state & XCB_MOD_MASK_4) && sym == XK_d) {
    ...
    if (pid == 0) {
      setsid();
      execlp("/home/hammad/wm/apps-launcher/apps-launcher.sh", "/home/hammad/wm/apps-launcher/apps-launcher.sh",
             NULL);
      exit(1);
    }
    if (wm.in_overview) {
...
void handle_button_release(xcb_button_release_event_t *ev) {
  ...
  if (wm.dragging) {
      ...
      if (wm.workspace_floating[ws] || is_force_float) {
        ...
        if (geo) {
          if (idx >= 0) {
            ...
            /*
             * IMPORTANT:
             * A force-floated window in REEL mode is positioned by:
             *
             *   slot_x + slide_dx
             *   slot_y + slide_dy
             *
             * Therefore every subsequent MOVE must update slide_dx/dy.
             * Previously only X/Y were updated here. On the next focus or
             * workspace layout, compute_reel_layout() reused the OLD offsets,
             * so the window jumped back to its previous position.
             */
            if (!wm.workspace_floating[ws]) {
              int16_t tx, ty;
              uint16_t tw, th;
              ...
              int32_t slot_x =
                  tx + (idx - focus_idx) * step - BORDER_WIDTH;

              wm.workspace_win_slide_dx[ws][idx] =
                  (int)geo->x - (int)slot_x;
              wm.workspace_win_slide_dy[ws][idx] =
                  (int)geo->y - (int)ty;
            }
          }
          free(geo);
    ...
    /*
     * RESIZE FIX FOR REEL/SLIDE MODE
     *
     * Resizing must only change W/H.
     * X/Y and slide_dx/slide_dy belong to the reel layout engine.
     *
     * The old code read geo->x/geo->y here and converted the resized window
     * into force_float. That is exactly what caused a resized slide window to
     * later restore at (0,0) after focus/workspace changes.
     */
    if (wm.dragging == 2 && wm.drag_window != XCB_WINDOW_NONE) {
      ...
      if (geo) {
        for (int ws = 0; ws < MAX_WORKSPACES; ws++) {
          ...
          /* Size is real client state. */
          wm.workspace_win_w[ws][i] = geo->width;
          wm.workspace_win_h[ws][i] = geo->height;

          /*
           * In floating mode position is client-owned.
           * In reel mode NEVER copy geo->x/y into workspace state and NEVER
           * change force_float or slide offsets merely because of a resize.
           */
          if (wm.workspace_floating[ws]) {
            wm.workspace_win_x[ws][i] = geo->x;
            wm.workspace_win_y[ws][i] = geo->y;
      ...
      /*
       * Re-apply canonical geometry immediately. This keeps the resized
       * window in its current reel slot and prevents later focus/workspace
       * changes from using a stale or temporary X/Y.
       */
      int ws = wm.current_workspace;
      if (ws >= 0 && ws < MAX_WORKSPACES &&
          !wm.workspace_tiling[ws] &&
...
static void grab_key_mod4_variants(xcb_keycode_t keycode, uint16_t mods) {
  /* Grab all common lock-state variants.  Without the Mod2 (usually NumLock)
   * variants, global Mod4 shortcuts can leak through to the focused client. */
  ...
  for (size_t i = 0; i < sizeof(variants) / sizeof(variants[0]); i++) {
    xcb_grab_key(wm.conn, 1, wm.root, variants[i], keycode,
...
void setup_key_grabs() {
  ...
  /* Grab every Mod4 shortcut handled in handle_key_press(). */
  xcb_keysym_t extra_mod4_keys[] = {XK_d, XK_e, XK_o, XK_n};
  for (size_t i = 0;
       i < sizeof(extra_mod4_keys) / sizeof(extra_mod4_keys[0]); i++) {
    xcb_keycode_t *kc =
...
void handle_configure_request(xcb_configure_request_event_t *ev) {
  for (int ws = 0; ws < MAX_WORKSPACES; ws++) {
    ...
    if (reel_mode) {
      ...
      if (ws != wm.current_workspace) {
        ...
        xcb_configure_window(wm.conn, ev->window, pmask,
                             (uint32_t *)pvalues);
      } else {
        compute_reel_layout(ws);
        apply_reel_geometry(ws);
     ...
     *
     * Keep the old behaviour for the CURRENT workspace.  This is
     * important because newly mapped floating windows receive their
     * initial X/Y from the WM's centering code.
     *
     * The special case is an INACTIVE floating workspace.  Firefox and
     * other single-instance applications can send ConfigureRequest with
     * X/Y while opening/reusing a page.  Those X/Y values can be based on
     * the window's parked off-screen position.  Never save/accept those
     * coordinates for an inactive floating window.
     */
    uint32_t mask = 0;
    uint32_t values[7];
    int n = 0;
    ...
    if (!inactive_floating &&
        (ev->value_mask & XCB_CONFIG_WINDOW_X)) {
    ...
    if (geo) {
      /* Always keep size synchronized. */
      wm.workspace_win_w[ws][idx] = geo->width;
      wm.workspace_win_h[ws][idx] = geo->height;

      /*
       * Only synchronize X/Y when the client was allowed to change them.
       * For inactive floating windows the WM's saved coordinates must
       * survive the client's ConfigureRequest.
       */
      if (!inactive_floating) {
        wm.workspace_win_x[ws][idx] = geo->x;
        wm.workspace_win_y[ws][idx] = geo->y;
    ...
    if (inactive_floating) {
      /*
       * Keep the inactive floating window parked, but DO NOT change its
       * saved WM coordinates.  When the workspace becomes current,
       * switch_workspace() can restore the saved centered/floating
       * position normally.
       */
      int32_t park_x =
          wm.workspace_win_x[ws][idx] + wm.screen_width * (ws + 2);
  ...
  if (ev->value_mask & XCB_CONFIG_WINDOW_X) {
    mask |= XCB_CONFIG_WINDOW_X; values[n++] = (uint32_t)(int32_t)ev->x;
  }
  if (ev->value_mask & XCB_CONFIG_WINDOW_Y) {
    mask |= XCB_CONFIG_WINDOW_Y; values[n++] = (uint32_t)(int32_t)ev->y;
  }
  if (ev->value_mask & XCB_CONFIG_WINDOW_WIDTH) {
    mask |= XCB_CONFIG_WINDOW_WIDTH; values[n++] = ev->width;
  }
  if (ev->value_mask & XCB_CONFIG_WINDOW_HEIGHT) {
    mask |= XCB_CONFIG_WINDOW_HEIGHT; values[n++] = ev->height;
  }
  if (ev->value_mask & XCB_CONFIG_WINDOW_BORDER_WIDTH) {
    mask |= XCB_CONFIG_WINDOW_BORDER_WIDTH; values[n++] = ev->border_width;
  }
  if (ev->value_mask & XCB_CONFIG_WINDOW_SIBLING) {
    mask |= XCB_CONFIG_WINDOW_SIBLING; values[n++] = ev->sibling;
  }
  if (ev->value_mask & XCB_CONFIG_WINDOW_STACK_MODE) {
    mask |= XCB_CONFIG_WINDOW_STACK_MODE; values[n++] = ev->stack_mode;
  }
  if (mask)
    xcb_configure_window(wm.conn, ev->window, mask, values);
  xcb_flush(wm.conn);
