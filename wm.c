...
void handle_map_request(xcb_map_request_event_t *ev) {
  ...
  if (geo && (wm.workspace_floating[ws] || force_float)) {
    ...
    // Center mein position set karein
    wm.workspace_win_x[ws][idx] = center_x - (geo->width / 2);
    wm.workspace_win_y[ws][idx] = center_y - (geo->height / 2);
  }
  if (geo) {
...
void handle_configure_request(xcb_configure_request_event_t *ev) {
  /*
   * IMPORTANT:
   * In reel/slide mode the WM owns window X/Y.  Client ConfigureRequests
   * are allowed to change size, but must never overwrite the carousel
   * position or turn a normal reel window into a forced-float window.
   */
  for (int ws = 0; ws < MAX_WORKSPACES; ws++) {
    ...
    int reel_mode = !wm.workspace_tiling[ws] && !wm.workspace_floating[ws];
    ...
    if (reel_mode) {
      ...
      if (ws != wm.current_workspace) {
        /* FIX: window ne khud raise/configure maanga hai (e.g. ek
         * single-instance app jaise Firefox ko file-manager se dobara
         * khola gaya) lekin ye kisi doosri (non-current) workspace par
         * hai. Pehle yahan seedha apply_reel_geometry(ws) call ho jati
         * thi jo is window ko uski PARKED (off-screen) position se
         * hata kar seedha on-screen coordinates par rakh deti thi -
         * chahe wo workspace abhi visible na ho - is se window current
         * (ghalat) tab par aa jati thi lekin kabhi raise/focus na hone
         * ki wajah se wahan ke apps ke PEECHE chhup jati thi. Ab is
         * ke bajaye uski asal workspace par slide kar ke le aate hain
         * aur usay focus bhi kar dete hain, taake wo sahi tab par aur
         * sabse upar nazar aaye. */
        switch_workspace(ws);
        focus_window(ev->window);
      } else {
        /* Re-assert the WM's authoritative reel position. */
        compute_reel_layout(ws);
        apply_reel_geometry(ws);
      }
      xcb_flush(wm.conn);
    ...
    /* Tiling/floating modes may use the normal client ConfigureRequest. */
    uint32_t mask = 0;
    uint32_t values[7];
    int n = 0;
