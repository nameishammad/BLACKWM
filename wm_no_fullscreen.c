...
void compact_workspaces(int removed_ws) {
  ...
  for (int i = removed_ws; i < wm.num_workspaces - 1; i++) {
    ...
    wm.workspace_prev_focus[i] = wm.workspace_prev_focus[i + 1];
    wm.workspace_last_active[i] = wm.workspace_last_active[i + 1];
  ...
  wm.workspace_prev_focus[last] = XCB_WINDOW_NONE;
  wm.workspace_last_active[last] = XCB_WINDOW_NONE;
...
static void repark_all_noncurrent_workspaces(void) {
  /*
   * Keep every non-current workspace in its own dedicated off-screen lane.
   * The logical coordinates in workspace_win_x/y stay local to the workspace;
   * only the physical X position gets the workspace parking offset.
   *
   * This is deliberately done for ALL windows, not only the moved one.
   * After a large workspace has been populated, some windows can otherwise
   * retain a stale physical position from a previous switch/overview pass
   * and become visible in another workspace.
   */
  for (int ws = 0; ws < wm.num_workspaces; ws++) {
    if (ws == wm.current_workspace)
      continue;
    ...
    for (int i = 0; i < wm.workspace_counts[ws]; i++) {
      ...
      int32_t x = wm.workspace_win_x[ws][i] + offset;
      int32_t y = wm.workspace_win_y[ws][i];

      uint32_t mask = XCB_CONFIG_WINDOW_X | XCB_CONFIG_WINDOW_Y;
      int32_t values[] = {x, y};
      xcb_configure_window(wm.conn, w, mask, (uint32_t *)values);
    }
  }
  xcb_flush(wm.conn);
}
...
void move_window_to_workspace(xcb_window_t win, int target_ws) {
  ...
  for (int i = 0; i < wm.dialog_count; i++) {
    ...
    if (wm.in_overview) {
      ...
      repark_all_noncurrent_workspaces();
    }
    xcb_flush(wm.conn);
    return;
  }
  ...
  if (wm.workspace_counts[target_ws] < MAX_WINDOWS) {
    ...
    /*
     * The destination slot must start from the saved logical geometry.
     * In particular, do not copy a parked physical X coordinate.
     */
    wm.workspace_win_x[target_ws][new_idx] = 0;
    wm.workspace_win_y[target_ws][new_idx] = 0;
    ...
    if (target_ws == wm.current_workspace) {
      ...
      uint32_t mask = XCB_CONFIG_WINDOW_X | XCB_CONFIG_WINDOW_Y;
      int32_t values[] = {
          wm.workspace_win_x[target_ws][new_idx] +
              wm.screen_width * (target_ws + 2),
          wm.workspace_win_y[target_ws][new_idx]};
      xcb_configure_window(wm.conn, win, mask, (uint32_t *)values);
    }
  ...
  if (from_ws == wm.current_workspace) {
    if (wm.workspace_counts[from_ws] > 0) {
      ...
      int f = wm.workspace_focus_idx[from_ws];
      if (f < 0 || f >= wm.workspace_counts[from_ws])
        f = wm.workspace_counts[from_ws] - 1;
      wm.workspace_focus_idx[from_ws] = f;

      if (f >= 0)
        focus_window(wm.workspace_windows[from_ws][f]);
    } else {
      compact_workspaces(from_ws);
      ...
      if (wm.workspace_counts[wm.current_workspace] > 0) {
        ...
        int f = wm.workspace_focus_idx[wm.current_workspace];
        if (f < 0 || f >= wm.workspace_counts[wm.current_workspace])
          f = wm.workspace_counts[wm.current_workspace] - 1;
        wm.workspace_focus_idx[wm.current_workspace] = f;

        if (f >= 0)
          focus_window(wm.workspace_windows[wm.current_workspace][f]);
      } else {
        wm.focused = XCB_WINDOW_NONE;
      }
    }
  ...
  /*
   * Final invariant:
   *   - current workspace windows are at their logical coordinates
   *   - every other workspace is physically parked in its own lane
   *
   * This is the important part for the "10+ windows then drag one" case.
   */
  if (!wm.in_overview)
    repark_all_noncurrent_workspaces();
