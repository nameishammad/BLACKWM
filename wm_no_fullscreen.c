...
void get_layout_target(int16_t *target_x, int16_t *target_y,
...
void compute_reel_layout(int ws) {
  ...
  get_layout_target(&target_x, &target_y, &target_w, &target_h);
...
void handle_map_request(xcb_map_request_event_t *ev) {
  ...
  if (ws != wm.current_workspace) {
    ...
    if (wm.workspace_floating[ws] || force_float) {
      ...
      int16_t fx, fy;
      get_layout_target(&fx, &fy, &target_w, &target_h);
    }
